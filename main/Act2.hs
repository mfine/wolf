...
import           Shelly hiding ( FilePath, (<.>), bash, cmd )
...
exec :: MonadIO m => String -> Uid -> Metadata -> [Blob] -> m (Metadata, [Artifact])
exec cmd' uid metadata blobs =
  shelly $ withDir $ \dir dataDir storeDir -> do
    ...
    return (result, artifacts) where
      ...
      bash dir = do
        ...
        cd dir
        maybe (return ()) (uncurry $ run_ . fromText) $ uncons $ words $ pack cmd'
